...
const EditTaskForm = ({ taskData, navigation, onCancel, onSuccess }) => {
  const queryClient = useQueryClient();
  const [taskName, setTaskName] = useState(taskData.name || "");
  const [taskCategory, setTaskCategory] = useState(
    taskData.taskCategory?.id || ""
  );
  ...
  const [criteria, setCriteria] = useState(
    taskData.taskCriterias?.map((item) => ({
      id: item.id,
      description: item.description || "",
      weight: item.weight || 1,
      taskCriteriaId: item.id, // Keep track of original ID for update
      isExisting: true,
    })) || [
      { id: 1, description: "", weight: 1, isExisting: false, isNew: true },
    ]
  );
  ...
  const batchId = taskData.batchTasks?.[0]?.batchId;
  const batchName = taskData.batchTasks?.[0]?.batchName;
  ...
  const updateTaskWithCreateAndUpdateCriteriaMutation = useMutation({
    mutationFn: (payload) =>
      updateTaskWithCreatedAndUpdateCriteria(
        ...
        payload.taskData,
        payload.criteriaList,
        payload.existingCriteriaIdList,
        payload.existingCriteriaList
      ),
    onSuccess: (data) => {
      setSuccessModal(true);
      queryClient.invalidateQueries({ queryKey: ["tasks"] });
    ...
    onError: (error) => {
      console.error(
        "Error updating task and creating new criteria:",
        error.message
      );
    },
  });
  ...
  const isLoading =
    ...
    createCriteriaMutation.isLoading ||
    deleteTaskCriteriaMutation.isLoading ||
    updateTaskWithCreateAndUpdateCriteriaMutation.isLoading;
  const isError =
    updateTaskMutation.isError ||
    createCriteriaMutation.isError ||
    deleteTaskCriteriaMutation.isError ||
    updateTaskWithCreateAndUpdateCriteriaMutation.isLoading;
  const error =
    updateTaskMutation.error ||
    createCriteriaMutation.error ||
    deleteTaskCriteriaMutation.error ||
    updateTaskWithCreateAndUpdateCriteriaMutation;
  ...
  const validateForm = () => {
    ...
    // Validate all criteria, both existing and new
  ...
  const handleAddCriteria = () => {
    setCriteria([
      ...criteria,
      {
        id: `new_${Date.now()}`, // Use timestamp for unique ID
        description: "",
        weight: 1,
        isNew: true,
        isExisting: false,
  ...
  const showDeleteConfirmation = (id) => {
    const criteriaItem = criteria.find((item) => item.id === id);
    setCriteriaToDelete(criteriaItem);
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (criteriaToDelete.isExisting) {
        try {
          await deleteTaskCriteriaMutation.mutateAsync(
            criteriaToDelete.taskCriteriaId
          );
          console.log(
            `Criteria ${criteriaToDelete.taskCriteriaId} deleted successfully`
          );
          ...
          console.error(
            `Failed to delete criteria ${criteriaToDelete.taskCriteriaId}:`,
            error
          );
        }
      }

      setCriteria(criteria.filter((item) => item.id !== criteriaToDelete.id));
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Separate existing and new criteria
      const existingCriteria = criteria.filter((item) => item.isExisting);
      const newCriteria = criteria.filter((item) => !item.isExisting);

      // Prepare task data according to the API spec
      const taskDataPayload = {
        name: taskName,
        taskCategoryId: taskCategory,
        taskCriteriasId: existingCriteria.map((item) => item.taskCriteriaId),
        batchTasks: [
          {
            assignedDate: formatDateToLocal(assignedDate),
      ...
      const criteriaList = existingCriteria.map((item) => ({
      ...
      if (newCriteria.length > 0) {
        ...
        const newCriteriaPayloads = newCriteria.map((item) => ({
          description: item.description.trim(),
          weight: parseInt(item.weight, 10) || 1,
        }));

        const payloadNewAndUpdateCriteria = {
          taskData: taskDataPayload,
          criteriaList: newCriteriaPayloads,
          existingCriteriaIdList: existingCriteria
            .map((item) => item.taskCriteriaId)
            .filter(Boolean),
          existingCriteriaList: criteriaList,
        };

        try {
          await updateTaskWithCreateAndUpdateCriteriaMutation.mutateAsync(
            payloadNewAndUpdateCriteria
          );
      ...
      const payload = {
        ...
        criteriaIdList: existingCriteria.map((item) => item.taskCriteriaId),
      };
    ...
    <ScrollView className="flex-1 bg-white">
      <View className="p-4">
        <View className="flex-row items-center mb-4">
          <TouchableOpacity onPress={onCancel} className="p-1 mr-2">
        ...
        {batchName && (
          <View className="bg-primary-50 p-3 rounded-lg mb-4 flex-row items-center">
            ...
            <Text style={fonts.ecTextBody2} className="text-primary-700 ml-2">
              Editing task for Batch: {batchName}
            </Text>
          </View>
        )}
        ...
        <View className="mb-6">
          ...
          {criteria.map((item, index) => (
            <Card
              key={item.id}
              title={`Criteria ${index + 1}${item.isNew ? " (New)" : ""}`}
              variant="neutral"
              className="mb-3"
              collapsible={false}
              action={
        ...
        <View className="flex-row justify-between mb-6">
          ...
          <Button
            ...
            loading={isLoading || isCreatingCriteria}
      ...
      >
        ...
        >
          <View className="bg-white p-6 rounded-lg w-5/6 max-w-md">
            ...
            >
              Task "{taskName}" has been updated successfully.
            </Text>
      ...
      >
        ...
        >
          <View className="bg-white p-6 rounded-lg w-5/6 max-w-md">
            <View className="items-center mb-4">
              <MaterialIcons name="warning" size={60} color="#CB3A31" />
              <Text
                style={fonts.ecTextHeader2M}
                className="text-alert-700 mt-2"
              >
                Confirm Delete
              </Text>
            </View>
